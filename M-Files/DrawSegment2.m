...
if numel(Extremas) > 3
    ...
    for k = 1 : length(Phases) - 3
            un = unique(Phases(k:k+3));
            if numel(un) == 4 && all(un>0) 
                Beziers = zeros(4,3);
                for l = 0 : 3
                    P_(:,:) = SkFrames(Extremas(k+l),:,:);
                    %return;
                    Nodes = [sign(P_(16,2))*P_(BezierOrder,2),P_(BezierOrder,1)];
                    BezierX = GetBezier(Nodes(:,1),100);
                    BezierY = GetBezier(Nodes(:,2),100);
                    Beziers(Phases(k+l),1) = std(BezierX);
                    Beziers(Phases(k+l),2) = std(BezierY);
                    Beziers(Phases(k+l),3) = (std(BezierY).^2+std(BezierX).^2) * 0.5;
                end;
